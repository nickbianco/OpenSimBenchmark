...
# Create Gait3D models
study.add_task(TaskCreateGait3DModels)
...
def add_model(model_file, label, flags=[]):
    ...
    # Benchmark tests.
    benchmark_euler = model.add_benchmark('benchmark_euler')
    benchmark_euler.add_task(TaskRunBenchmark, model.tasks[-1],
            exe_args={'time': time, 'accuracy': accuracy})
    benchmark_euler.add_task(TaskPlotBenchmark, benchmark_euler.tasks[-1])

    benchmark_rk4 = model.add_benchmark('benchmark_rk4')
    benchmark_rk4.add_task(TaskRunBenchmark, model.tasks[-1],
            exe_args={'time': time, 'accuracy': accuracy})
    benchmark_rk4.add_task(TaskPlotBenchmark, benchmark_rk4.tasks[-1])

    benchmark_cpodes = model.add_benchmark('benchmark_cpodes')
    benchmark_cpodes.add_task(TaskRunBenchmark, model.tasks[-1],
            exe_args={'time': time, 'accuracy': accuracy})
    benchmark_cpodes.add_task(TaskPlotBenchmark,
            benchmark_cpodes.tasks[-1])
