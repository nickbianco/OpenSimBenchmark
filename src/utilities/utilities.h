...
void add_controller(OpenSim::Model& model) {
    ...
    for (const auto& actu : model.getComponentList<ScalarActuator>()) {
        SimTK::Real minControl = actu.getMinControl();
        SimTK::Real maxControl = actu.getMaxControl();
        Constant function(0.2);
        controller->prescribeControlForActuator(
                actu.getAbsolutePathString(), function);
    }
...
void saveMapToJsonFile(const std::unordered_map<std::string, int>& map, 
